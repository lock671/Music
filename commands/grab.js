...
module.exports = new Command({
	...
	async run(message, args, client, slash) {
        ...
        if (!queue || !queue.playing) {
            return message.reply({ embeds: [{ description: `There's nothing currently playing in the server.`, color: 0x36393e }], ephemeral: true });
        }
		if (slash)
			message.reply({ embeds: [{ description: `Sent a private message!`, color: 0x36393e }], ephemeral: true });
		else
			message.react('📩');
