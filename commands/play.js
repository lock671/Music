...
module.exports = new Command({
	...
	async run(message, args, client, slash) {
        if(!message.member.voice.channelId)
            return message.reply({ embeds: [{ description: `You are not in a voice channel!`, color: 0x36393e }], ephemeral: true, failIfNotExists: false });
        if(message.guild.me.voice.channelId && message.member.voice.channelId !== message.guild.me.voice.channelId)
            return message.reply({ embeds: [{ description: `You are not in my voice channel!`, color: 0x36393e }], ephemeral: true, failIfNotExists: false });
        if(!args[0]) return;
        ...
        if (!searchResult || !searchResult.tracks.length) {
            reply = { embeds: [{ description: `No results found!`, color: 0x36393e }], ephemeral: true, failIfNotExists: false };
        ...
        try {
            ...
            reply = { embeds: [{ description: `Could not join your voice channel!`, color: 0x36393e }], failIfNotExists: false };
            slash ? message.editReply(reply) : message.reply(reply);
            return;
        }
