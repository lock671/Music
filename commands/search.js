...
module.exports = new Command({
	...
	async run(message, args, client, slash) {
        ...
        if (!searchResult || !searchResult.tracks.length) {
            const reply = { embeds: [{ description: `No results found!`, color: 0x36393e }], ephemeral: true, failIfNotExists: false };
            slash ? message.editReply(reply) : message.reply(reply);
            return;
        }     
		if (searchResult.playlist) {
			const reply = { embeds: [{ description: `This command does not support playlists.\nUse **${client.prefix}play** instead.`, color: 0x36393e }], ephemeral: true, failIfNotExists: false };
            slash ? message.editReply(reply) : message.reply(reply);
            return;
		}
		const buttons = [];
